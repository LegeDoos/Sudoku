    ...
    public class Puzzle
    {
        ...
        public bool SolvePuzzle()
        {
            ...
            if (!IsSolved)
            {
                ...
                List<(int, int, int)> possibleValuesCount = []; // i, j, count
                for (int i = 0; i < 9; i++)
                {
                    for (int j = 0; j < 9; j++)
                    {
                ...
                int index = 0;
                while (!subSolved && index < possibleValues.Length)
                {
        ...
        public void Print(bool showPossibleValues)
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                ...
                for (int j = 0; j < 9; j++)
                {
                    ...
                    var c = j % 3 == 0 && j != 0 ? '|' : ' ';
                    lineSet = $"{lineSet}{c}{Values[i, j].Value,-9}";
                    lineValues = $"{lineValues}{c}{Values[i, j].ShowPossibleValues()}";
                }
                lineSet = $"{lineSet} ";
                lineValues = $"{lineValues} ";
                Console.WriteLine("".PadLeft(lineValues.Length, i % 3 == 0 && i != 0 ? '=' : ' '));
        ...
        public void SetValue(int x, int y, int value, bool skipEnqueue = false, bool skipGenerateSuggestions = false)
        {
            // checks
            if (x < 0 || x > 8)
                throw new ArgumentOutOfRangeException(nameof(x), "Invalid position");

            if (y < 0 || y > 8)
                throw new ArgumentOutOfRangeException(nameof(y), "Invalid position");
